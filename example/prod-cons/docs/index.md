...
whenever asked to review a certain resource.
	...
	│   │   ├── tran_in     # files fed to ZOTI-Tran + (debug) preprocessed by ZOTI-YAML
	│   │   └── tran_out    # files generated by ZOTI-Tran + (debug) preprocessed by ZOTI-YAML
...
librarires). For ZOTI-Gen inputs (see `gen/<example>/tran_out/*.zoc`)
...
connecting each auxiliary component. We inkvoke again the [principle
...
counter probes and monitor would performa a too specialized
...
that most ports have a `data_type` entry containing either a `{name:
<type-name>, ...}` or a `{from_ftn: <type-definition>, ...}`
entry. Check the ones specifying a type name formed as
...
copying, etc) and the access macros we just mentioned in the prevoius
paragraph.
